...
func main() {
	...
	g := gen.NewGenerator(gen.Config{
		...
		OutPath: "./internal/dal/query", //curd代码的输出路径
	...
	autoCreateTimeField := gen.FieldGORMTag("create_time", "column:create_time;type:bigint unsigned;autoCreateTime")
	//softDeleteField := gen.FieldType("delete_time", "gorm.DeletedAt")
	softDeleteField := gen.FieldType("deleted_at", "soft_delete.DeletedAt")
	softDeleteFieldTag := gen.FieldGORMTag("deleted_at", "column:deleted_at;type:int;softDelete:milli")

	// 模型自定义选项组
	//fieldOpts := []gen.ModelOpt{jsonField, autoCreateTimeField, autoUpdateTimeField, softDeleteField}
	fieldOpts := []gen.ModelOpt{autoCreateTimeField, softDeleteField, softDeleteFieldTag}
